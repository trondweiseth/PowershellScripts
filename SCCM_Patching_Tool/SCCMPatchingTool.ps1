...
$objRichTextBox1.ForeColor    = 'Green'
...
$RemoveSCCMCache.Add_Click({ removesccmcache })
...
$hashstring = (Get-Content $scriptlocation) | where { $_ -match "objTextBox4.Text             = '" -and $_ -notmatch 'replace' }
...
$descriptionstring  = (Get-Content $scriptlocation) | where { $_ -match "objTextBox2.Text             = '" -and $_ -notmatch 'replace' }
...
$appversionstring  = (Get-Content $scriptlocation) | where { $_ -match "objTextBox5.Text             = '" -and $_ -notmatch 'replace' }
...
Function Display-Message {
    ...
    $objRichTextBox1.ForeColor = 'Black'
    $objRichTextBox1.Text = $MessageString
    $objTextBox1.Focus()
}
...
Function Display-Error {
    ...
    $objRichTextBox1.Text = $ErrorString
    $objTextBox1.Focus()
}

Function serverstatus() {
    if ($objTextBox1.Lines) {
        Display-Message -MessageString "Checking server status...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
...
Function restartserver() {
    if ($objTextBox1.Lines) {
        Display-Message -MessageString "Restarting server(s)...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
...
Function stopserver() {
    if ($objTextBox1.Lines) {
        Display-Message -MessageString "Stopping server(s)...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
...
Function removecheckpoints() {
    if ($objTextBox1.Lines) {
        Display-Message -MessageString "Removing checkpoint(s)...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
...
Function coldcheckpoint() {
    ...
    if ($objTextBox1.Lines) {
        Display-Message -MessageString "Creating checkpoint...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
...
Function getchekcpoints() {
    if ($objTextBox1.Lines) {
        Display-Message -MessageString "Getting checkpoint(s)...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
    $objTextBox1.Lines | ForEach-Object {
        ...
        if ($checkpoints) {
            ...
            Display-Error -ErrorString "No chekcpoint(s).`n"
        }
    }
}
...
Function startServer() {
    if ($objTextBox1.Lines) {
        Display-Message -MessageString"Starting server(s)...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
...
Function runSCCMClientAction {
    ...
    if ($objTextBox1.Lines) {
        Display-Message -MessageString"Running SCCM action: ${ClientAction}...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
...
Function removesccmcache() {
    if ($objTextBox1.Lines) {
        Display-Message -MessageString "Removing SCCM cache...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
    $objTextBox1.Lines | ForEach-Object {
        try {
            ...
            if ($_.Exception.Message -imatch 'winrm') {
                Display-Error -ErrorString "Error connecting to remote server`nError message : WinRM cannot process the request.`n"
            }
            else {
                Display-Error -ErrorStrin "Unforseen error. Please try again.`n"
            }
        }
    }
}
...
Function appinstallation() {
    ...
    if ($objTextBox1.Lines) {
        Display-Message -MessageString "Starting app installation...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
...
Function installstatus() {
    ...
    $appversion = $objTextBox5.Text
    if ($objTextBox1.Lines) {
        Display-Message -MessageString "Checking installation status...`n"
    }
    else {
        Display-Error -ErrorString "ERROR: No host(s).`n"
    }
